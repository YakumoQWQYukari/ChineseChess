...
public class Chessman_bing extends Chessman {
    private int id;
    private Point point;
    ...
    private  Chessman_bing chessman_bing = null;//是否被选中--处理器
    //棋子图像
    private Circle circle = new Circle();
    private ImageView imageView1 = new ImageView(new Image(new File("src\\resources\\image\\RB.png").toURI().toString()));
    private ImageView imageView2 = new ImageView(new Image(new File("src\\resources\\image\\BZ.png").toURI().toString()));
//    private ImageView imageView1 = new ImageView(new Image(new File(Entrance.class.getResource("/resources/image/RB.png").toString()).toString()));
//    private ImageView imageView2 = new ImageView(new Image(new File(Entrance.class.getResource("/resources/image/BZ.png").toString()).toString()));

    public Chessman_bing(Point point,int id) {
        //初始化对象
        this.point = point;
        this.id = id;
...
//                System.out.println(Chessman.mutex != null && Chessman.mutex != this && Chessman.mutex.getEdibleChess().size() != 0
    ...
    public void dispose(double x,double y){
        ...
        point.setOnMouseClicked(e -> {
            if(chessman_bing != null){
                //把当前对象移动到对应点位
                chessman_bing.getCircle().setCenterX((int)point.getCenterX());
                chessman_bing.getCircle().setCenterY((int)point.getCenterY());
