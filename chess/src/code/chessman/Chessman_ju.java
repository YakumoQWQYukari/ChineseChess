...
public class Chessman_ju extends Chessman {
    private int id;
    private Point point;
    ...
    private Chessman_ju chessman_ju = null;//是否被选中--处理器
    //棋子图像
    private Circle circle = new Circle();
    private ImageView imageView1 = new ImageView(new Image(new File("src\\resources\\image\\RC.png").toURI().toString()));
    private ImageView imageView2 = new ImageView(new Image(new File("src\\resources\\image\\BC.png").toURI().toString()));
//    private ImageView imageView1 = new ImageView(new Image(new File(Entrance.class.getResource("/resources/image/RC.png").toString()).toString()));
//    private ImageView imageView2 = new ImageView(new Image(new File(Entrance.class.getResource("/resources/image/BC.png").toString()).toString()));

    public Chessman_ju(Point point,int id) {
        //初始化对象
        this.point = point;
        this.id = id;
    ...
    public void dispose(double x,double y){
        ...
        point.setOnMouseClicked(e -> {
            if(chessman_ju != null){
                //把当前对象移动到对应点位
                chessman_ju.getCircle().setCenterX((int)point.getCenterX());
                chessman_ju.getCircle().setCenterY((int)point.getCenterY());
                //虚拟数组改变，并把虚拟棋盘数组加入列表
