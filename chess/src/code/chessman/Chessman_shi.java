...
public class Chessman_shi extends Chessman {
    private int id;
    private Point point;
    ...
    private Chessman_shi chessman_shi = null;//是否被选中--处理器
    //棋子图像
    private Circle circle = new Circle();
    private ImageView imageView1 = new ImageView(new Image(new File("src\\resources\\image\\RS.png").toURI().toString()));
    private ImageView imageView2 = new ImageView(new Image(new File("src\\resources\\image\\BS.png").toURI().toString()));
//    private ImageView imageView1 = new ImageView(new Image(new File(Entrance.class.getResource("/resources/image/RS.png").toString()).toString()));
//    private ImageView imageView2 = new ImageView(new Image(new File(Entrance.class.getResource("/resources/image/BS.png").toString()).toString()));


    public Chessman_shi(Point point,int id) {
        //初始化对象
        this.point = point;
        this.id = id;
...
//                System.out.println(Chessman.mutex != null && Chessman.mutex != this && Chessman.mutex.getEdibleChess().size() != 0
    ...
    public void dispose(double x,double y){
        ...
        point.setOnMouseClicked(e -> {
            if(chessman_shi != null){
                //把当前对象移动到对应点位
                chessman_shi.getCircle().setCenterX((int)point.getCenterX());
                chessman_shi.getCircle().setCenterY((int)point.getCenterY());
