...
public class Chessman_xiang extends Chessman {
    private int id;
    private Point point;
    ...
    private Chessman_xiang chessman_xiang = null;//是否被选中--处理器
    //棋子图像
    private Circle circle = new Circle();
    private ImageView imageView1 = new ImageView(new Image(new File("src\\resources\\image\\RX.png").toURI().toString()));
    private ImageView imageView2 = new ImageView(new Image(new File("src\\resources\\image\\BX.png").toURI().toString()));
//    private ImageView imageView1 = new ImageView(new Image(new File(Entrance.class.getResource("/resources/image/RX.png").toString()).toString()));
//    private ImageView imageView2 = new ImageView(new Image(new File(Entrance.class.getResource("/resources/image/BX.png").toString()).toString()));

    public Chessman_xiang(Point point,int id) {
        //初始化对象
        this.point = point;
        this.id = id;
    ...
    public void removablePoints(Point p){
        ...
        if(y <= 5){
            ...
            if(x>= 1 && x<=9 && y>=1 && y<=5){
                ...
                if(Chessboard.chessBoard[(int)y-1+1][(int)x-1-1] == null){
                    dispose(x,y);
                }
            ...
            if(x>= 1 && x<=9 && y>=1 && y<=5){
                ...
                System.out.println(Chessboard.chessBoard[(int)y-1][(int)x+1] == null);
                if(Chessboard.chessBoard[(int)y-1-1][(int)x-1+1] == null){
                    dispose(x,y);
                }
            }
    ...
    public void dispose(double x,double y){
        ...
        point.setOnMouseClicked(e -> {
            if(chessman_xiang != null){
                //把当前对象移动到对应点位
                chessman_xiang.getCircle().setCenterX((int)point.getCenterX());
                chessman_xiang.getCircle().setCenterY((int)point.getCenterY());
